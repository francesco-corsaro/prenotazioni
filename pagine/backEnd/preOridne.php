...
if ($clean==1) {
    ...
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()){
            if ($row['email']==$email) {
                echo '<p class="etichetta marginSup marginInf">Hai gi� effettuato la registrazione per questo giorno</p>' ;
                $persons=9999;
                $conn->close();
                break;
            } 
    ...
    echo '<p class="etichetta marginSup marginInf">
                Per completare la prenotazione è necessario fare clicl sul link che riceverai all\'indirizzo '.$email.'
                </p>';
